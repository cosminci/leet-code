...
object _2127_MaxEmployeesToBeInvitedToMeeting:
  def maximumInvitations(favorite: Array[Int]): Int =
    ...
    def topologicalSort(queue: Seq[Int]): Unit =
      ...
      indegree(fav) -= 1
      if indegree(fav) > 0 then topologicalSort(queue.tail)
      else
        visited(fav) = 1
    ...
    def maxLength(person: Int, length: Int): Int =
      if visited(person) == 1 then length
      else
        visited(person) = 1
        maxLength(favorite(person), length + 1)

    val (type1, type2) = favorite.zipWithIndex.foldLeft(0, 0) { case ((type1, type2), (fav, person)) =>
      if visited(person) == 1 then (type1, type2)
      else
        val length = maxLength(person, length = 0)
        if length == 2 then (type1 + dp(person) + dp(fav) + 2, type2)
